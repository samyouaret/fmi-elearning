...
class CurriculumController extends Controller
{
   ...
   public function show(int $id)
   {
      ...
      $course = DB::table("course_chapter")->select(
            "course_chapter.id as chapter_id",
            "chapter_title",
   ...
   public function createChapter(int $id)
   {
      ...
      $id = DB::table("course_chapter")->insertGetId($data);
      $data['chapter_id'] = $id;
   ...
   public function updateChapter(Request $request, int $id)
   {
      ...
      return DB::table('course_chapter')->where('id', $id)->update($data) ?
         response()->json(['status' => 'success', 'message' => "title has been updated successfully"], 200) : response()->json(['status' => 'failed', 'message' => "title has not been changed"], 413);
   }
