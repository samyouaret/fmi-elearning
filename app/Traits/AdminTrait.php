...
trait AdminTrait
{
   ...
   public function addSubject(Request $request)
   {
     ...
     return $id ? response()->json(
        ["id"=>$id,'status'=>'success','message' =>"subject added."],200):
        response()->json(
   ...
   public function getSubSubjects()
   {
     ...
     ->select('sub_subject.id as sub_id','sub_subject.id as id',
     'sub_subject.label as sub_label','subject.label as label')
   ...
   public function addSubSubject(Request $request)
   {
     ...
     return DB::table('sub_subject')->insert(['label'=>$sub_subject,
      'subject_id'=>$subject_id]) ? response()->json(
        ['status'=>'success','message' =>"sub subject added."],200):
        response()->json(
           ['status'=>'failed','message' =>"somthing went wrong try again."],422);
   }
}
